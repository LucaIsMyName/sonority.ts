...
export const Current: React.FC<{ 
  ...
  ({ children, className }) => (
    <CurrentContextProvider className={className}>
...
const createSubcomponent = (
  propName: keyof TrackProps, 
  defaultRenderer?: (track: TrackProps) => React.ReactNode
) => {
  return ({ className, children }: { 
    ...
    return currentTrack && currentTrack[propName] ? (
      <div className={className}>{currentTrack[propName]}</div>
    ) : null;
  };
};
